...
export default async function BookingPage({ params, searchParams }) {
  ...
  return (
    <div 
      className={isEmbed ? "w-full cal-wrapper" : "min-h-screen bg-[#0a0a0a] flex items-center justify-center p-4 sm:p-6 cal-wrapper"}
      style={{
        '--cal-accent': themeColor,
        '--cal-radius': radiusVal,
      }}
    >
      <div className={`cal-container w-full ${isEmbed ? 'max-w-none bg-transparent' : 'max-w-4xl bg-[#111] rounded-2xl shadow-2xl border border-white/10'} overflow-hidden flex flex-col md:flex-row relative`}>
        
        {/* Left Side: Event Details */}
        <div className={`cal-sidebar w-full md:w-1/3 ${isEmbed ? 'bg-black/20' : 'bg-black/40'} p-8 border-b md:border-b-0 md:border-r border-white/10 flex flex-col relative overflow-hidden`}>
          {/* Theme Color Accent */}
          <div 
          ...
          <div className="mb-6 flex flex-col items-center text-center">
            {calendar.logoUrl ? (
              <div className="cal-logo w-24 h-24 mb-4 rounded-xl overflow-hidden flex items-center justify-center bg-white/5 border border-white/10 p-2 shadow-inner">
                <img src={calendar.logoUrl} alt={`${calendar.name} Logo`} className="w-full h-full object-contain" />
              </div>
            ) : (
              <div className="cal-logo w-12 h-12 rounded-xl bg-white/5 flex items-center justify-center mb-4 border border-white/10">
                <CalendarIcon className="w-6 h-6 text-white" />
              </div>
            )}
            <h1 className="cal-title text-2xl font-bold text-white mb-2">{calendar.name}</h1>
          ...
          {calendar.description && (
            <div 
              className="cal-description mt-6 pt-6 border-t border-white/10 prose prose-sm prose-invert max-w-none text-text-tertiary"
