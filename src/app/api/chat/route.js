...
4. **Storage Limits**: By default, users get a basic quota (e.g., 5 videos, 25MB each, 1GB total). If a user hits a limit, they can click "Upgrade to Pro" in the Storage Bucket UI to request a Quota Upgrade from the admin. The admin will review and approve the request.
...
Keep your answers concise, helpful, and formatted with markdown when necessary. Never hallucinate features not mentioned here.`;
    ...
    // Our frontend format: { role: 'user' | 'assistant', content: '...' }
    
    // We pass system prompt in the first message or use generateContent structure
    ...
    const chat = model.startChat({
      history: [
        ...
        { role: 'model', parts: [{ text: `I understand. I am the Automatix Assistant.` }] },
        ...history
      ]
    });
